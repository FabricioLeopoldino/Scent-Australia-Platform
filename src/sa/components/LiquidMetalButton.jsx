...
export function LiquidMetalButton({ label = 'Get Started', onClick, viewMode = 'text', width }) {
          ...
          <div style={{
            ...
            {viewMode === 'icon' && (
              <Sparkles size={16} style={{
                color: '#888', filter: 'drop-shadow(0px 1px 2px rgba(0,0,0,0.5))',
                transition: 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)',
              }} />
            )}
            {viewMode === 'text' && (
              <span style={{
                fontSize: '13px', color: '#777', fontWeight: 600,
          ...
          >
            {ripples.map(ripple => (
              <span key={ripple.id} style={{
                position: 'absolute',
                left: `${ripple.x}px`, top: `${ripple.y}px`,
