      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20, marginBottom: 28 }}>
          {[
            ...
            { label: 'Low Stock Items', value: stats.low_stock,      sub: stats.low_stock > 0 ? 'Below minimum level' : 'All healthy', color: stats.low_stock > 0 ? '#f59e0b' : '#22c55e', icon: <TrendingDown size={18} color={stats.low_stock > 0 ? '#f59e0b' : '#22c55e'} />, path: '/stock', tip: 'Produtos com stock actual abaixo do nível mínimo definido.' },
            { label: 'Pending POs',     value: stats.pending_pos,    sub: stats.pending_pos > 0 ? 'Awaiting receipt' : 'No open orders', color: stats.pending_pos > 0 ? '#fb923c' : '#22c55e', icon: <ShoppingCart size={18} color={stats.pending_pos > 0 ? '#fb923c' : '#22c55e'} />, path: '/incoming-orders', tip: 'Purchase orders com status pending ou partial — aguardam recepção de mercadoria.' },
