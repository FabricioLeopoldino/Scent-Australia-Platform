...
const pool = new Pool({
  ...
  ssl: PLATFORM_URL.includes('localhost') ? false : { rejectUnauthorized: true },
...
async function runStartupMigrations() {
  ...
  // `bom_rules` (pre-D9, superseded by product_bom) is no longer created — its routes
  // were removed 2026-07-30 and the production table sits empty. Existing databases keep
  // the empty table; nothing reads it.
